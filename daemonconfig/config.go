...
func Default() Config {
	return Config{Version: Version, Log: LogConfig{Level: "info", Output: "stderr"}, Control: ControlConfig{REST: RESTConfig{Listen: "127.0.0.1:9090"}}, Session: SessionConfig{ID: "default", AutoStart: true, AutoReconnect: true}, ATrust: ATrustConfig{Port: 443, AuthType: "auth/psw", LoginDomain: "Radius", UpdateBestNodesInterval: Duration(30 * time.Second)}, Underlay: UnderlayConfig{AutoDetect: true}, Routing: RoutingConfig{Mode: core.RoutingModeRule, InternetOutbound: InternetOutboundConfig{Type: core.InternetOutboundDirect}}, DNS: DNSConfig{Remote: RemoteDNSConfig{Enabled: true, Server: "auto"}, CacheTTL: Duration(time.Hour), Hosts: map[string]string{}}, Inbounds: InboundsConfig{SOCKS5: SOCKS5Config{Listen: "127.0.0.1:1080"}, HTTP: HTTPConfig{Listen: "127.0.0.1:1081"}, TUN: TUNConfig{Name: "ZJU-Portal", Address: "172.19.0.1/30", MTU: 1400, UDP: TUNUDPConfig{IdleTimeout: Duration(60 * time.Second), MaxFlows: 4096}, DNS: TUNDNSConfig{FakeIPRange: "198.18.0.0/16"}}}}
}
