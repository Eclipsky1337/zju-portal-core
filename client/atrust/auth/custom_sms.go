...
func (s *Session) completeCustomSMS(ctx context.Context) (authStep, error) {
	if err := s.sendCustomSMS(ctx); err != nil {
		return authStep{}, err
	}
	...
	return s.customSMSCheckCode(ctx, response.Value, response.Skip)
}

func (s *Session) sendCustomSMS(ctx context.Context) error {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", u+"?"+params.Encode(), bytes.NewReader(body))
	if err != nil {
		return err
	}
...
func (s *Session) customSMSCheckCode(ctx context.Context, code string, skipSecondaryAuth bool) (authStep, error) {
	log.Println("Perform POST /passport/v1/auth/customSms")
