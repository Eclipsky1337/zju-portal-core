...
func (m CASLogin) login(ctx context.Context, s *Session, authInfo AuthInfo) error {
	return s.loginAuthCas(ctx, authInfo.LoginURL, m.Domain, m.Ticket)
}

func (s *Session) loginAuthCas(ctx context.Context, loginURL, loginDomain, ticket string) error {
	callback := s.casCallbackFromTicket(loginDomain, ticket)
	if ticket == "" {
		var err error
		callback, err = s.interactiveCas(ctx, loginURL)
		if err != nil {
			return err
		}
	}

	if err := s.cas(ctx, callback); err != nil {
		return err
	}
...
func (s *Session) interactiveCas(ctx context.Context, loginURL string) (string, error) {
	response, err := s.handleChallenge(ctx, core.AuthChallenge{
		ID:     s.randSdpId(16),
		Kind:   core.AuthChallengeCASCallback,
		Prompt: "Enter CAS callback URL: ",
		URL:    loginURL,
	})
	if err != nil {
		return "", err
...
func (s *Session) cas(ctx context.Context, callback string) error {
