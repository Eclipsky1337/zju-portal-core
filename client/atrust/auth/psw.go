...
func (m PasswordLogin) login(ctx context.Context, s *Session, _ AuthInfo) error {
	return s.loginAuthPsw(ctx, m.Username, m.Password, m.Domain, m.GraphCodeFile)
}

func (s *Session) loginAuthPsw(ctx context.Context, username, password, loginDomain, graphCodeFile string) error {
	process := func(graphCheckCode string) (int, error) {
		return s.pswImpl(ctx, username, password, loginDomain, graphCheckCode)
	}
	return s.withGraphCheckCode(ctx, process, graphCodeFile)
}

func (s *Session) pswImpl(ctx context.Context, username, password, loginDomain, graphCheckCode string) (int, error) {
