...
func (s *Session) authConfig(ctx context.Context, mod, needTicket bool) (int, []AuthInfo, error) {
...
func (s *Session) reportEnv(ctx context.Context) error {
...
func (s *Session) authCheck(ctx context.Context) (authStep, error) {
...
func (s *Session) phoneNumber(ctx context.Context, authID string) ([]string, error) {
...
func (s *Session) authSms(ctx context.Context, step authStep) error {
...
func (s *Session) smsCheckCode(ctx context.Context, step authStep) (authStep, error) {
	...
	return s.secondarySMSCheckCodeImpl(ctx, step, response.Value, response.Skip)
}

func (s *Session) secondarySMSCheckCodeImpl(ctx context.Context, step authStep, code string, skipSecondaryAuth bool) (authStep, error) {
	u := s.baseURL + "/passport/v1/auth/sms"
	params := WithSharedParams(url.Values{
		"action": {"checkcode"},
	})
...
func (s *Session) onlineInfo(ctx context.Context) (string, error) {
...
func (s *Session) ClientResource() ([]byte, error) {
	return s.ClientResourceContext(context.Background())
}

func (s *Session) ClientResourceContext(ctx context.Context) ([]byte, error) {
...
func (s *Session) checkCode(ctx context.Context) ([]byte, error) {
	...
	body, err := io.ReadAll(io.LimitReader(resp.Body, core.MaxAuthChallengeImageSize+1))
	if err != nil {
		return nil, err
	}
	if len(body) > core.MaxAuthChallengeImageSize {
		return nil, fmt.Errorf("check code image exceeds %d bytes", core.MaxAuthChallengeImageSize)
	}
