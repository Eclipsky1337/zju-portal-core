...
func (c *Client) tcpResourceFor(ctx context.Context, addr *net.TCPAddr) (string, string, string) {
	appID := ""
	nodeGroupID := ""
	domain := ""
	if resource, ok := ctx.Value(resolve.ContextKeyDomainResource).(client.DomainResource); ok {
		...
		return appID, nodeGroupID, domain
	}
...
func (c *Client) DialTCP(ctx context.Context, addr *net.TCPAddr) (net.Conn, error) {
	appID, nodeGroupID, domain := c.tcpResourceFor(ctx, addr)
	if appID == "" {
		return nil, fmt.Errorf("no matching aTrust TCP resource with appId for %s", addr)
	}
