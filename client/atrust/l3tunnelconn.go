...
var (
	errL3TunnelAuthTimeout = errors.New("l3-tunnel auth timeout")
	errL3HeartbeatTimeout  = errors.New("l3-tunnel heartbeat timeout")
)
	...
	for {
		...
		case <-ticker.C:
			if err := c.writeFrame([]byte{l3Version, cmdHeartbeatReq, 0x00, 0x00}); err != nil {
				c.fail(fmt.Errorf("write l3-tunnel heartbeat: %w", err))
				return
			}
		case <-c.closeCh:
			return
...
func (c *l3TunnelConn) heartbeatWatchdogLoop() {
	ticker := time.NewTicker(heartbeatInterval)
	defer ticker.Stop()
	c.watchHeartbeats(ticker.C)
}

func (c *l3TunnelConn) watchHeartbeats(ticks <-chan time.Time) {
	for {
		select {
		case now := <-ticks:
			if c.heartbeatExpired(now) {
				c.fail(errL3HeartbeatTimeout)
				return
			}
		case <-c.closeCh:
			return
...
func (c *l3TunnelConn) heartbeatExpired(now time.Time) bool {
	lastHeartbeat := time.Unix(0, c.lastHeartbeat.Load())
	return now.Sub(lastHeartbeat) >= heartbeatTimeout
}
...
func defaultEnv(info clientInfo) *trustEnv {
	procPath := "/usr/bin/zju-portal-core"
	procName := "zju-portal-core"
